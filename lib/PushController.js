...
var send = function (pushAssociations, androidPayload, iosPayload,wpPayload) {
    ...
    var mpnsTokens = _(pushAssociations).where({type: 'wp'}).map(function(user){ return {token:user.token,delay:user.delay,delayDate:user.delayDate}}).value();
    ...
    if (wpPayload && wpTokens.length > 0) {
        var mpnsPayload = mpnsPusher.buildPayload(wpPayload);
        mpnsPusher.push(mpnsTokens, mpnsPayload);
    }
};
